...
const Post = ({
  ...
  const [localCommentNo, setLocalCommentNo] = useState(commentNo);

  const navigate = useNavigate();

  const updateCommentNo = () => {
    setLocalCommentNo((prev) => prev + 1);
  };

  // Clicar fora de avatar/nome/comentários leva para post
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-lg mb-4 max-w-[50rem]">
      ...
      <div className="flex items-center gap-2">
        <Link to={`/profile/${userId}`} className="bg-[#0B3D26] w-12 h-12 rounded-full">
      ...
      {/* Conteúdo clicável que leva ao post */}
      <div
      ...
      >
        {/* Text content */}
        {textContent && (
          <p className="mt-2 text-lg break-words whitespace-pre-wrap ">{textContent}</p>
        )}
      ...
      <div className="mt-4 flex justify-between items-center">
        ...
        <button
          onClick={(e) => {
            e.stopPropagation(); // evita que clique propague para o container do post
      ...
      {openComments && (
        <Comments postId={id} close={() => setOpenComments(false)} updateCommentNo={updateCommentNo} />
      )}
    </div>
  );
};
